...
function updateUI(d){
  ...
  // ФИКС: раньше статус определялся по rssi_dbm!==0, что ложно показывало
  // "не подключено" при реальном RSSI ровно 0 dBm. Теперь используем явный
  // флаг sta_connected из /api/status.
  $('dot').className='status-dot'+(d.sta_connected?' ok':'');
}
