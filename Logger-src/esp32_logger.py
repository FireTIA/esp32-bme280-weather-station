...
def sync_history(logger: Logger, settings: dict):
    ...
    # base_timestamp будет маленьким числом (аптайм). Вычисляем время относительно ПК!
    if base_ts < 1000000000 and status_data and "uptime_sec" in status_data:
        current_pc_ts = time.time()
        esp_uptime = status_data["uptime_sec"]
        # Вычисляем реальный epoch для oldest point
        real_base_ts = current_pc_ts - (esp_uptime - base_ts)
    else:
        real_base_ts = base_ts

    recovered_count = 0
    for i, pt in enumerate(points):
        # Точное время конкретной точки
        point_ts = real_base_ts + (i * interval)
